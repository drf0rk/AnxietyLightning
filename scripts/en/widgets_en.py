# /content/ANXETY/scripts/en/widgets_en.py (v20.0 - State Persistence & Styling Fix)
...
class AnxietyUI:
    def __init__(self):
        ...
        # --- NEW: Persistent state for selections ---
        self.selections = {
            'model_list': set(),
            'vae_list': set(),
            'controlnet_list': set(),
    ...
    def _create_widgets(self):
        # This method is now much simpler as lists are handled by the update function
    ...
    def _create_layouts(self):
        ...
        # Layouts for model lists are now initialized empty
    ...
    def _save_current_selections(self):
        """Iterates through visible checkboxes and updates the persistent selection sets."""
    ...
    def _update_model_lists(self):
        """Re-creates the checkbox lists while preserving selections."""
        self._save_current_selections() # Save state before rebuilding
        ...
        layout_map = {
            'model_list': self.layouts['models_box'],
            'vae_list': self.layouts['vaes_box'],
            'controlnet_list': self.layouts['cnets_box'],
            'lora_list': self.layouts['loras_box']
        }
    ...
    def _build_and_display_review_stage(self):
        ...
        for url in self.url_pool:
            data = self.api.get_data(url) if "civitai.com" in url else self._get_huggingface_guesses(url)
            if data:
                # Add a version list for Civitai data
                if "civitai.com" in url: data['versions'] = self.api.get_model(url).model_versions
        ...
        for i, item in enumerate(self.processed_data):
            ...
            # --- ENHANCED FILE/VERSION SELECTION ---
            row_widgets = [name_label, type_dropdown, base_model_dropdown]
            if not is_hf and item.get('versions'):
                versions_map = {v.name: v for v in item['versions']}
                version_dropdown = self.factory.create_dropdown(options=list(versions_map.keys()), description="Model Version:")
                
                initial_version_obj = versions_map[version_dropdown.value]
                file_options = {f.name: f.download_url for f in initial_version_obj.files}
            ...
            # --- STYLING FIX: Use layout properties instead of CSS ---
    ...
    def _structure_civitai_data(self, data, url):
        # Now fetches the full model to get all versions
        full_model_data = self.api.get_model(url)
        return {'source': 'Civitai', 'name': data.get('model', {}).get('name'), 'type': data.get('model', {}).get('type'),
                'baseModel': data.get('baseModel'), 'files': data.get('files', []), 'versions': full_model_data.model_versions if full_model_data else []}
    ...
    def save_settings(self):
        self._save_current_selections() # Make sure the final state is saved
        print("--- 1. Saving All UI Settings ---")
        SETTINGS_PATH = ANXETY_ROOT / 'settings.json'
        # Save from the persistent selection sets, not the widgets
        widget_values = {key: list(value) for key, value in self.selections.items()}
        # Add other widget values
